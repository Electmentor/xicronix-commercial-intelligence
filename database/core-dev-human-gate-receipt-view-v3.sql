-- CORE DEV ONLY. Preserve real MFA and admin gate on all reads.
    ...
    'adaptive_queue', coalesce((
      ...
      from (
        select
          ...
          q.risk,
          q.execution_id,
          q.experiment_approval_id,
          (select ev.payload - 'session_id' from core.events ev
           where ev.organization_id=q.organization_id and ev.correlation_id=q.id
             and ev.event_type='adaptive.human_gate_decision'
             and ev.payload->>'approval_id'=q.experiment_approval_id::text
           order by ev.created_at desc limit 1) as approval_receipt,
          q.learning_approval_id,
          q.institutional_knowledge_id,
          q.evidence_id,
        ...
        where q.organization_id=v_org
          and q.status in ('awaiting_approval','approved_dev','running_dev','awaiting_learning_approval','learned','rolled_back')
