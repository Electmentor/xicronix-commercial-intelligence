...
with sync_playwright() as p:
 ...
 try:
  ...
  row=page.locator('tr',has_text='Institución de validación · Diagnóstico');row.get_by_role('button',name='Editar',exact=True).click()
  ...
  row=page.locator('tr',has_text='Diagnóstico revisado de prueba');row.get_by_role('button',name='Registrar interacción',exact=True).click()
  expect(field('institution_id')).to_have_value('inst-1');expect(field('contact_id')).to_have_value('contact-1');expect(field('type')).to_have_value('');expect(field('outcome')).to_have_value('');expect(field('subject')).to_have_value('');mark('new interaction inherits links but never fabricates call subject or outcome')
  ...
  field('type').select_option('WEB_FORM');field('subject').fill('Registro ficticio de formulario');field('need_summary').fill('Necesidad de diagnóstico de laboratorio');field('decision_timeline').fill('Por confirmar');field('budget_signal').fill('No informado');field('notes').fill('Texto de prueba\n<em>sin ejecutar HTML</em>');field('occurred_at').fill('2026-09-18T13:53');field('next_action').fill('Preparar reunión');
  ...
  nav('activities');page.locator('button[data-edit="'+new['id']+'"]').click();expect(field('type')).to_have_value('WEB_FORM');expect(field('outcome')).to_have_value('');expect(field('notes')).to_have_value(new['notes']);expect(field('decision_timeline')).to_have_value('Por confirmar');expect(field('need_summary')).to_have_value('Necesidad de diagnóstico de laboratorio');expect(field('next_action_date')).to_have_value('2026-09-22T11:30');mark('reopening edited interaction retains every saved field')
  page.screenshot(path=str(out/'linked-interaction-fixture.png'),full_page=True);cancel()
  page.locator('button[data-edit="activity-1"]').click();expect(field('outcome')).to_have_value('');field('subject').fill('Recepción web revisada');page.locator('#saveBtn').click();saved();assert page.evaluate('window.__testDB.activities[0].occurred_at')=='2026-09-18T12:00:50.327Z';mark('editing another field preserves original event seconds and milliseconds')
  nav('leads');page.locator('tr',has_text='Diagnóstico revisado de prueba').get_by_role('button',name='Ver solicitud').click();expect(page.locator('#leadDetailContent')).to_contain_text('Formulario web');page.locator('#leadDetailContent button[data-edit-activity="activity-1"]').click();expect(page.locator('#leadDetailDialog')).not_to_be_visible();expect(page.locator('#editor')).to_be_visible();mark('original request opens its own edit form without duplicate events');cancel()
  nav('leads');page.locator('tr',has_text='Diagnóstico revisado de prueba').get_by_role('button',name='Registrar interacción').click();field('type').select_option('EMAIL');field('subject').fill('Prueba de integridad');field('institution_id').select_option('inst-2');field('contact_id').select_option('contact-2');before=page.evaluate('window.__testWrites.length');page.locator('#saveBtn').click();expect(page.locator('#formMsg')).to_contain_text('institución debe coincidir');assert page.evaluate('window.__testWrites.length')==before;mark('mismatched prospect institution is blocked');cancel()
  nav('tasks');page.locator('button[data-edit="task-1"]').click();expect(field('lead_id')).to_be_visible();expect(field('contact_id')).to_be_visible();field('lead_id').select_option('33333333-3333-4333-8333-333333333333');expect(field('contact_id')).to_have_value('contact-1');mark('task editor also supports consistent links');cancel()
  for width in [390,768,1440]:
   page.set_viewport_size({'width':width,'height':950});nav('leads');page.locator('tr',has_text='Diagnóstico revisado de prueba').get_by_role('button',name='Registrar interacción').click();expect(field('type')).to_be_visible();assert page.locator('#editor').evaluate('(el)=>el.scrollWidth<=el.clientWidth+1');cancel();mark('editor fits viewport '+str(width))
