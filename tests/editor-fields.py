...
with sync_playwright() as p:
 ...
 try:
  ...
  expect(field('institution_id')).to_have_value('inst-1');expect(field('contact_id')).to_have_value('contact-1');expect(field('action_code')).to_have_value('');expect(field('type')).to_have_value('');expect(field('outcome')).to_have_value('');expect(field('subject')).to_have_value('');expect(page.locator('#movementPreview')).to_contain_text('Solo los hitos definidos');field('action_code').select_option('PROPOSAL_PRESENTED');expect(page.locator('#movementPreview')).to_contain_text('50%');mark('new movement starts empty and explains milestone impact before saving')
  ...
  nav('leads');page.locator('#recordList tr',has_text='Diagnóstico revisado de prueba').get_by_role('button',name='Abrir expediente comercial').click();expect(page.locator('#leadDetailContent')).to_contain_text('Madurez comercial');expect(page.locator('#leadDetailContent')).to_contain_text('15%');expect(page.locator('#leadDetailContent')).to_contain_text('Prospecto identificado y calificado');expect(page.locator('#leadDetailContent')).to_contain_text('Formulario web');page.locator('#leadDetailContent button[data-edit-activity="activity-1"]').click();expect(page.locator('#leadDetailDialog')).not_to_be_visible();expect(page.locator('#editor')).to_be_visible();mark('original request opens its own edit form without duplicate events');cancel();expect(page.locator('#leadDetailDialog')).to_be_visible();mark('cancel returns to originating prospect dossier');page.locator('#closeLeadDetail').click()
  ...
  nav('leads');page.locator('#recordList tr',has_text='Diagnóstico revisado de prueba').get_by_role('button',name='Abrir expediente comercial').click();expect(page.locator('#leadDetailContent')).to_contain_text('Documentos del expediente');expect(page.locator('#leadDetailContent')).to_contain_text('Diagnóstico técnico de prueba');expect(page.locator('#leadDetailContent')).to_contain_text('Historial de versiones (2)');page.locator('#closeLeadDetail').click();mark('commercial dossier summarizes current document and version history')
