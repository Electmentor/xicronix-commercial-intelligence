...
with sync_playwright() as p:
 ...
 try:
  ...
  row=page.locator('#recordList tr',has_text='Diagnóstico revisado de prueba');row.get_by_role('button',name='Registrar interacción',exact=True).click()
  ...
  for width in [390,768,1440]:
   page.set_viewport_size({'width':width,'height':950});nav('leads');page.locator('#recordList tr',has_text='Diagnóstico revisado de prueba').get_by_role('button',name='Registrar interacción').click();expect(field('type')).to_be_visible();assert page.locator('#editor').evaluate('(el)=>el.scrollWidth<=el.clientWidth+1');cancel();mark('editor fits viewport '+str(width))
