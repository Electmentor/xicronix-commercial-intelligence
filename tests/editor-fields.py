...
with sync_playwright() as p:
 ...
 try:
  ...
  nav('leads');page.locator('#recordList tr',has_text='Diagnóstico revisado de prueba').get_by_role('button',name='Ver solicitud').click();expect(page.locator('#leadDetailContent')).to_contain_text('Madurez comercial');expect(page.locator('#leadDetailContent')).to_contain_text('15%');expect(page.locator('#leadDetailContent')).to_contain_text('Prospecto identificado y calificado');expect(page.locator('#leadDetailContent')).to_contain_text('Formulario web');page.locator('#leadDetailContent button[data-edit-activity="activity-1"]').click();expect(page.locator('#leadDetailDialog')).not_to_be_visible();expect(page.locator('#editor')).to_be_visible();mark('original request opens its own edit form without duplicate events');cancel()
