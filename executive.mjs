...
export function renderExecutive(data,{now=new Date(),demo=false,failures={},analyticsPeriod='year'}={}){
 ...
 const mobileClone='<section class="director-mobile-dashboard director-mobile-clone" aria-label="Dashboard de dirección móvil">'+
  ...
  '</section>'+
  '<section class="director-mobile-section director-mobile-section--clone"><header><h3>Pipeline</h3><small>Estado actual</small></header><div class="director-mobile-pipeline director-mobile-pipeline--clone">'+mobilePipeline.map(([label,count,color],index)=>{const icons=[
 '<svg viewBox="0 0 24 24" aria-hidden="true"><circle cx="8" cy="8" r="3"/><circle cx="16" cy="9" r="2.5"/><path d="M3.5 19v-2.2A4.8 4.8 0 0 1 8.3 12h.4a4.8 4.8 0 0 1 4.8 4.8V19"/><path d="M13.5 13.2a4.3 4.3 0 0 1 6.8 3.5V19"/></svg>',
 ...
 '<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M6 3h9l3 3v15H6z"/><path d="M14 3v4h4"/><path d="M9 11h6M9 15h6"/></svg>',
 '<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M3.5 12.2 7 8.7l4.2 4.2M20.5 12.2 17 8.7l-4.2 4.2"/><path d="m8.8 12.9 2.2 2.2a1.6 1.6 0 0 0 2.3 0l2.5-2.5"/><path d="m6.2 14.1 3.1 3.1M17.8 14.1l-3.1 3.1"/></svg>',
 '<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M8 4h8v4a4 4 0 0 1-8 0V4Z"/><path d="M8 6H5a2.5 2.5 0 0 0 2.5 3H8M16 6h3a2.5 2.5 0 0 1-2.5 3H16"/><path d="M12 12v4M9 20h6M10 16h4"/></svg>'
 ][index];return '<div class="director-pipeline-stage director-pipeline-stage--clone" style="--pipe:'+color+'"><i class="stage-accent" aria-hidden="true"></i><div class="stage-main"><span class="stage-icon stage-icon--glow">'+icons+'</span><strong>'+count+'</strong></div><small>'+label+'</small></div>';}).join('')+'</div></section>'+
  '<section class="director-mobile-section director-mobile-section--clone director-mobile-prospects"><header><h3>Prospectos potenciales</h3><button data-page="prospects">Ver todos →</button></header><button class="director-mobile-prospect-feed" data-page="prospects"><span><strong>'+actionNow.length+'</strong><small>acción ahora</small></span><span><strong>'+reviewFirst.length+'</strong><small>por investigar</small></span><span><strong>'+prospects.length+'</strong><small>candidatos</small></span><span class="clone-chevron">›</span></button></section>'+
  '<section class="director-mobile-section director-mobile-section--clone"><header><h3>Requiere mi atención</h3></header><div class="director-mobile-exceptions director-mobile-exceptions--clone">'+(m.decisions[0]?'<button data-edit="'+esc(m.decisions[0].id)+'" data-table="'+m.decisions[0].table+'"><span class="attention-icon"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M6 17h12l-1.5-2.2V10a4.5 4.5 0 0 0-9 0v4.8L6 17Z"/><path d="M10 20h4"/></svg></span><span><strong>'+esc(m.decisions[0].title)+'</strong><small>'+esc(m.decisions[0].detail)+'</small></span><span class="clone-chevron">›</span></button>':'<button data-page="now" class="clone-empty-row"><span class="attention-icon"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M6 17h12l-1.5-2.2V10a4.5 4.5 0 0 0-9 0v4.8L6 17Z"/><path d="M10 20h4"/></svg></span><span><strong>Sin excepciones críticas.</strong></span><span class="clone-chevron">›</span></button>')+'</div></section>'+
  '<section class="director-mobile-section director-mobile-section--clone"><header><h3>Equipo comercial</h3></header><div class="director-mobile-team director-mobile-team--clone">'+(m.team.length?'<button data-page="users"><span class="team-icon"><svg viewBox="0 0 24 24" aria-hidden="true"><circle cx="9" cy="8" r="3"/><circle cx="16" cy="9" r="2.5"/><path d="M4 19v-2a5 5 0 0 1 5-5h.3a5 5 0 0 1 5 5v2M14 13a4 4 0 0 1 6 3.4V19"/></svg></span><span><strong>'+m.team.length+' ejecutivo'+(m.team.length===1?'':'s')+' comercial'+(m.team.length===1?'':'es')+'</strong><small>'+m.team.reduce((s,row)=>s+row.openCount,0)+' casos activos · '+m.team.reduce((s,row)=>s+row.overdueTasks,0)+' tareas vencidas</small></span><span class="clone-chevron">›</span></button>':'<button data-page="users" class="clone-empty-row"><span class="team-icon"><svg viewBox="0 0 24 24" aria-hidden="true"><circle cx="9" cy="8" r="3"/><circle cx="16" cy="9" r="2.5"/><path d="M4 19v-2a5 5 0 0 1 5-5h.3a5 5 0 0 1 5 5v2M14 13a4 4 0 0 1 6 3.4V19"/></svg></span><span><strong>Sin equipo registrado.</strong></span><span class="clone-chevron">›</span></button>')+'</div></section>'+  '<section class="director-mobile-section director-mobile-section--clone director-mobile-radar"><header><h3>Radar comercial</h3><button data-page="radar">Abrir Radar →</button></header><div class="director-mobile-radar-card"><span class="radar-glow-icon"><svg viewBox="0 0 24 24" aria-hidden="true"><circle cx="12" cy="12" r="8"/><circle cx="12" cy="12" r="3"/><path d="M12 4v8l5 3"/></svg></span><div><strong>'+radarCritical.length+' críticas · '+radarHigh.length+' alta prioridad</strong><small>'+(topSignal?esc(topSignal.institution_name||'Señal prioritaria'):'Sin señal destacada')+'</small></div><span class="clone-chevron">›</span></div></section>'+
  '<section class="director-mobile-section director-mobile-section--clone director-mobile-upcoming"><header><h3>Próximos hitos</h3><button data-page="meetings">Ver agenda →</button></header><div class="director-mobile-upcoming-list">'+(upcoming.length?upcoming.slice(0,3).map(item=>'<button data-page="'+item.page+'"><span class="upcoming-date">'+new Date(item.date).toLocaleDateString('es-PE',{day:'2-digit',month:'short'})+'</span><span><strong>'+esc(item.title)+'</strong><small>'+esc(item.type)+'</small></span><span class="clone-chevron">›</span></button>').join(''):'<div class="director-empty">Sin hitos próximos.</div>')+'</div></section>'+
