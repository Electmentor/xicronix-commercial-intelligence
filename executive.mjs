...
export function renderExecutive(data,{now=new Date(),demo=false,failures={},analyticsPeriod='year'}={}){
 ...
 const mobileClone='<section class="director-mobile-dashboard director-mobile-clone" aria-label="Dashboard de dirección móvil">'+
  ...
  '</section>'+
  '<section class="director-mobile-section director-mobile-section--clone"><header><h3>Pipeline</h3><small>Estado actual</small></header><div class="director-mobile-pipeline director-mobile-pipeline--clone">'+mobilePipeline.map(([label,count,color],index)=>{const glyph=['◉','●','⌕','▤','🤝','♛'][index];return '<div class="director-pipeline-stage director-pipeline-stage--clone" style="--pipe:'+color+'"><span class="stage-accent"></span><span class="stage-glyph">'+glyph+'</span><strong>'+count+'</strong><small>'+label+'</small></div>';}).join('')+'</div></section>'+
  '<section class="director-mobile-section director-mobile-section--clone"><header><h3>Requiere mi atención</h3></header><div class="director-mobile-exceptions director-mobile-exceptions--clone">'+(m.decisions.slice(0,3).map(item=>'<button data-edit="'+esc(item.id)+'" data-table="'+item.table+'"><span class="attention-icon">♢</span><span><strong>'+esc(item.title)+'</strong><small>'+esc(item.detail)+'</small></span><span class="clone-chevron">›</span></button>').join('')||'<button data-page="now" class="clone-empty-row"><span class="attention-icon">♧</span><span><strong>Sin excepciones críticas.</strong></span><span class="clone-chevron">›</span></button>')+'</div></section>'+
  '<section class="director-mobile-section director-mobile-section--clone"><header><h3>Equipo comercial</h3></header><div class="director-mobile-team director-mobile-team--clone">'+(m.team.slice(0,4).map(row=>'<button data-ceo-seller="'+esc(row.user.id)+'"><span class="team-icon">♟</span><span><strong>'+esc(row.user.full_name)+'</strong><small>'+row.openCount+' casos · '+row.overdueTasks+' vencidas</small></span><b>'+(row.attainment===null?'—':Math.round(row.attainment)+'%')+'</b><span class="clone-chevron">›</span></button>').join('')||'<button data-page="users" class="clone-empty-row"><span class="team-icon">♟</span><span><strong>Sin equipo registrado.</strong></span><span class="clone-chevron">›</span></button>')+'</div></section>'+
  '<nav class="director-mobile-bottom-nav" aria-label="Accesos rápidos de dirección">'+
   '<button class="active" data-page="dashboard"><span>⌂</span><small>Inicio</small></button>'+
   '<button data-page="opportunities"><span>▣</span><small>Oportunidades</small></button>'+
   '<button data-page="goals"><span>▤</span><small>Reportes</small></button>'+
   '<button data-page="now"><span>♧</span><small>Notificaciones</small></button>'+
  '</nav>'+
 '</section>';
